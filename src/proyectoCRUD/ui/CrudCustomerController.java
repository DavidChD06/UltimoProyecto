...
/**
 * Controller class for the Customer CRUD view. Handles the interaction between
 * the UI and the logic layer for managing Customers.
 *
 * @author david
...
public class CrudCustomerController implements Initializable, MenuActionsHandler {
    ...
    /**
     * Initializes the controller, sets up the stage, and configures the table
     * columns. Defines cell factories and edit commit handlers for data
    ...
    /**
     * Handles the action event when the exit button is clicked or the window is
     * closed. Shows a confirmation alert before closing the stage.
     *
    ...
    /**
     * Listener for selection changes in the customer table. Enables or disables
     * the delete button based on whether an item is selected.
     *
    ...
    /**
     * Handles the action event when the delete button is clicked. Validates if
     * the customer has associated accounts or is an administrator before
     * deletion. Asks for user confirmation before removing the customer from
    ...
    /**
     * Handles the action event when the refresh button is clicked. Reloads the
     * customer data from the server into the table.
     *
     * @param event The event that triggered this action.
     */
    ...
    /**
     * Handles the action event when the add button is clicked. Creates a new
     * empty customer in the database, adds it to the table, and focuses on the
     * new row.
     *
     * @param event The event that triggered this action.
     */
    ...
    /**
     * Fetches all customers from the server using the REST client and updates
     * the TableView items.
     */
